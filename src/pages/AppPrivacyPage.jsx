...
export default function AppPrivacyPage({ defaultId }) {
  ...
  const effectiveDate = app.effectiveDate || 'August 20, 2026';
  const collectionLines = getPolicyLines(app.collectedInfo, 'Account and device data necessary to deliver this application.');
  const useLines = getPolicyLines(app.howUsed, 'Provide, secure, and improve the application services you request.');
  const securityLines = getPolicyLines(app.storageSecurity, 'Data is protected with reasonable technical and organizational safeguards.');
  const rightsLines = getPolicyLines(app.userRights, `Request access, correction, export, or deletion by contacting ${CONTACT_EMAIL}.`);
      ...
      <main className="privacy-page__content">
        ...
        <header className="privacy-hero">
          ...
          <div className="privacy-hero__identity">
            <div className="privacy-app-icon"><AppLogo app={app} /></div>
            <div><p className="privacy-kicker"><ShieldCheck size={14} /> Official privacy policy</p><h1>{app.name}</h1><p className="privacy-hero__summary">A clear guide to how this application handles information, protects it, and gives you control.</p></div>
          </div>
          <div className="privacy-hero__meta"><span>{app.category || 'Asivision application'}</span><p><Clock size={14} /> Effective {effectiveDate}</p></div>
        </header>
        ...
        {(app.previewImageUrl || app.description) && (
          <section className="privacy-about-card">
            {app.previewImageUrl && <img src={app.previewImageUrl} alt={`${app.name} preview`} />}
            <div><p className="privacy-kicker"><Sparkles size={14} /> About the application</p><h2>Designed for {app.name} users</h2><p>{app.description || 'This policy applies to the application and the services it provides.'}</p>
              {app.linkUrl && app.linkUrl !== 'https://' && <a href={app.linkUrl} target="_blank" rel="noreferrer" className="privacy-text-link">Open application <ExternalLink size={14} /></a>}
            </div>
          </section>
        )}

        {app.disclaimers && <aside className="privacy-notice-card"><span className="privacy-notice-card__icon"><AlertTriangle size={18} /></span><div><strong>Important application notice</strong><p>{app.disclaimers}</p></div></aside>}

        <section className="privacy-policy-card">
          <div className="privacy-policy-card__intro"><p className="privacy-kicker"><FileText size={14} /> Your information, explained</p><h2>Privacy at a glance</h2><p>This policy explains how <strong>{app.name}</strong>, operated by Asivision Technology &amp; Innovation, collects, uses, stores, and protects information when you use the application or its related services.</p></div>
